...
---------hive中insert+values----------------
create table if not exists t_test_insert(id int,name string,age int);
...
--加载数据
load data local inpath '/opt/data/students.txt' into table student;

select * from student;

--step2：创建一张目标表  只有两个字段
create table if not exists student_from_insert(sno int,sname string);
...
LOAD DATA INPATH '/data/students.txt' INTO TABLE student_HDFS_p partition(country ="China");
