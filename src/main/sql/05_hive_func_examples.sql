...
--加载数据
load data local inpath '/opt/data/test01.txt' into table singer;
...
load data local inpath '/opt/data/apache_web_access.log' into table apachelog;
...
load data local inpath '/opt/data/test01_wash.txt' into table singer_wash;
...
--如果表已存在就删除表
drop table if exists itheima.singer;
--创建表
create table itheima.singer(id string,--歌手id
                    name string,--歌手名称
                    country string,--国家
                    province string,--省份
                    gender string,--性别
                    works string)--作品
ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.RegexSerDe' --指定使用RegexSerde加载数据
...
--加载数据
load data local inpath '/opt/data/url.txt' into table tb_url;
...
--加载数据到表中
load data local inpath '/opt/data/r2c1.txt' into table row2col1;
...
--加载数据到表中
load data local inpath '/opt/data/r2c2.txt' into table row2col2;
...
--加载数据
load data local inpath '/opt/data/c2r1.txt'  into table col2row1;
...
--加载数据
load data local inpath '/opt/data/c2r2.txt' into table col2row2;
...
--加载数据
load data local inpath '/opt/data/device.json' into table tb_json_test1;
...
load data local inpath '/opt/data/device.json' into table tb_json_test2;
...
load data local inpath '/opt/data/money.tsv' into table tb_money;
...
load data local inpath '/opt/data/emp.txt' into table tb_emp;
...
--加载模拟数据
load data local inpath '/opt/data/zipper.txt' into table dw_zipper;
--查询
...
load data local inpath '/opt/data/update.txt' into table ods_zipper_update;
